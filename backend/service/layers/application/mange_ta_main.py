...
def most_recipes_contributors(df_recipes: pd.DataFrame) -> pd.DataFrame:
    ...
    number_recipes_contributors = (
        df_recipes.groupby('contributor_id').size(
...
def best_ratings_contributors(
    ...
    avg_ratings = (
        df_interactions.groupby("recipe_id")[
            "rating"].mean().reset_index(name="avg_rating")
    )

    df = df_recipes.merge(avg_ratings, how="left",
                          left_on="id", right_on="recipe_id")

    contributor_counts = df.groupby("contributor_id")[
        "id"].count().reset_index(name="num_recipes")

    contributor_avg = df.groupby("contributor_id")[
        "avg_rating"].mean().reset_index()

    contributor_stats = contributor_avg.merge(
        contributor_counts, on="contributor_id")
    ...
    contributor_stats = contributor_stats.where(
        pd.notna(contributor_stats), None)
...
class DataAnylizer:
    ...
    def process_data(self, analysis_type: AnalysisType):
        match analysis_type:
            case AnalysisType.NO_ANALYSIS:
