...
def normalize_ids(df: pd.DataFrame, data_type: DataType) -> pd.DataFrame:
    ...
    match data_type:
        case DataTypes.INTERACTIONS:
            df['user_id_normalized'], uniques = pd.factorize(df['user_id'])
            df['user_id_normalized'] += 1
           
        case DataTypes.RECIPES:
            df['user_id_normalized'], uniques = pd.factorize(df['contributor_id'])
            df['user_id_normalized'] += 1
            df['recipe_id_normalized'], uniques = pd.factorize(df['id'])
            df['recipe_id_normalized'] += 1
