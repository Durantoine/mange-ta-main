...
# Permet les imports absolus de ton projet
...
render_sidebar()
st.title("⏱️ Répartition des durées des recettes")
st.caption("Distribution par tranches de minutes (0–15, 15–30, …, 120+)")

# Paramètres : vue en Nombre vs Pourcentage
...
try:
    ...
    struct_logger.error("Failed to fetch duration distribution", error=str(e))
else:
    if not data:
...
try:
    ...
    struct_logger.error("Failed to fetch duration vs recipe count", error=str(e))
else:
    st.divider()
